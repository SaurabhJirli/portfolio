...
export default async function TagPage({ params }: { params: { tag: string } }) {
  ...
  const posts = allPosts.filter((post) =>
    ((post.data as { tags?: string[] }).tags ?? []).includes(params.tag)
  );
    ...
    <section>
      <Wrapper className="py-24">
        <div className="text-center">
          ...
          >
            Blog posts
            <span className="block italic tracking-wide font-display"> tagged with {params.tag}</span>
          </Text>
          <Text
            tag="p"
            variant="textLG"
          ...
          >
            Find all the posts tagged with {params.tag}.
          </Text>
        </div>
